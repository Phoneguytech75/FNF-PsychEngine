...
class Alphabet extends FlxSpriteGroup
{
	...
	public var isInFreeplay:Bool = false;
	...
	{
		if (isMenuItem && isInFreeplay)
		{
			var scaledY = FlxMath.remapToRange(targetY, 0, 1, 0, 1.3);

			var lerpVal:Float = CoolUtil.boundTo(elapsed * 9.6, 0, 1);
			y = FlxMath.lerp(y, (scaledY * yMult) + (FlxG.height * 0.2) + yAdd, lerpVal);
			if(forceX != Math.NEGATIVE_INFINITY) {
				x = forceX;
			} else {
				x = FlxMath.lerp(x, 50 + xAdd, lerpVal);
			}
		}
		else if (isMenuItem)
		{
